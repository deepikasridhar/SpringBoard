...
select facid, name as "facility name", membercost as "member cost", monthlymaintenance as "monthly maintainance" from Facilities where membercost > 0 and membercost < (0.2 * monthlymaintenance)

/* Q4: How can you retrieve the details of facilities with ID 1 and 5?
Write the query without using the OR operator. */
select facid, name as "facility name", membercost as "member cost", monthlymaintenance as "monthly maintainance" ,guestcost,initialoutlay from Facilities where facid in (1,5)
...
select distinct m.surname , f.name from Facilities f,Bookings b, Members m
where b.memid=m.memid and b.facid in (0,1) and f.facid=b.facid
order by m.surname
